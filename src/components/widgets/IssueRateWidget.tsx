    ...
    <div className="h-full flex flex-col justify-center space-y-4">
      {MOCK_ISSUE_RATES.map((item, idx) => (
        <div key={idx} className="flex items-center gap-4">
          <div className="w-24 text-sm text-slate-700 dark:text-slate-300 truncate">{item.model}</div>
          ...
          <div className="w-12 text-right flex items-center justify-end gap-1">
            <span className="text-sm font-medium text-slate-800 dark:text-slate-200">{item.rate}%</span>
